...
- **Add**: drop the GGUF under `models_dir`, add an entry with a `seed`,
  regenerate, restart `llama-swap` and `llamacracy`. Benchmark it later with
  `phase0_bench.py --only <key>` and regenerate again.
- **Change** serving flags: edit `serve`, regenerate, restart. If the change
  affects speed or VRAM, re-benchmark with `--only <key> --fresh`.
- **Remove**: delete the entry, regenerate, restart. Its billing history
